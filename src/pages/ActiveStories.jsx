...
import { Search, MapPin, Store, Eye, Trash2, ImageOff, RefreshCw, PlusCircle } from 'lucide-react';

export default function ActiveStories() {
  // 1. جعل المصفوفة فارغة افتراضياً لتعكس الحالة الأولى للمشروع
  const [stories, setStories] = useState([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCity, setSelectedCity] = useState('');
  const [selectedMerchant, setSelectedMerchant] = useState('');

  // دالة لإضافة بيانات تجريبية مؤقتاً لتجربة الكروت عند الحاجة
  const addMockData = () => {
    setStories([
      {
        id: 1,
  ...
  const handleDeleteStory = (id) => {
    if (confirm('هل أنت متأكد من رغبتك في حذف/تجميد هذه الـ Story المخالفة؟')) {
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        {/* زر تجريبي لإضافة عروض ومعاينة الشاشة عند وجود بيانات */}
        <button
          onClick={addMockData}
      ...
      {/* العرض الشرطي: القائمة الفارغة أو الكروت */}
      {filteredStories.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredStories.map((story) => (
            <div
              key={story.id}
              className="bg-white rounded-2xl p-4 shadow-sm border border-stone-100 flex flex-col justify-between hover:shadow-md transition"
            >
              <div>
                <div className="relative h-48 rounded-xl overflow-hidden mb-4">
                  ...
                  />
                  <div className="absolute top-3 left-3 bg-black/70 text-white text-xs px-3 py-1 rounded-full flex items-center gap-1.5 backdrop-blur-sm">
                    <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse"></span>
                    <span>متبقي {story.timeLeft}</span>
                  </div>
                </div>
